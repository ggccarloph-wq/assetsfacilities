...
<div class="surface p-3 mb-3">
    ...
    <div class="request-actions mb-3" style="justify-content:flex-start">
        @if($reservation->isPending() && $reservation->isPrePlotted() && !$reservation->isVenueApproved())
            <form method="POST" action="{{ route('fmo.reservations.approve-venue', $reservation) }}">@csrf
        ...
        @if($canRejectHere)
            <button class="btn-reject" type="button" data-bs-toggle="collapse" data-bs-target="#reject-box"><i class="bi bi-x-lg"></i> Reject Request</button>
        @endif

        {{-- Panel feature: release a confirmed venue at short notice without
             touching a single approval. Only offered once every approver has
             signed, because that is the case Reject cannot handle. --}}
        @if($user->canManageFacilities() && $reservation->canBeEmergencyCancelled())
            <button class="btn-reject" type="button" data-bs-toggle="collapse" data-bs-target="#emergency-box"><i class="bi bi-exclamation-octagon"></i> Emergency Cancellation</button>
        @endif

        @if($user->canDeleteFacilityRecords())
            <form method="POST" action="{{ route('fmo.reservations.destroy', $reservation) }}" data-confirm="Delete this request everywhere? The linked Activity Proposal and Reservation will both be removed.">
                @csrf @method('DELETE')
    ...
    <div class="collapse mb-3" id="reject-box">
        <form method="POST" action="{{ route('fmo.reservations.reject', $reservation) }}" class="p-3" style="background:var(--surface-2);border:1px solid var(--line);border-radius:0">
            @csrf
            <label class="form-label">Reason for rejection</label>
            <textarea name="rejection_reason" class="form-control mb-2" rows="2" placeholder="e.g. Venue already assigned to another activity."></textarea>
            <button class="btn-reject"><i class="bi bi-x-lg"></i> Confirm Rejection</button>
        </form>
    </div>
    @endif

    {{-- ================== EMERGENCY CANCELLATION (FMO) ================== --}}
    @if($user->canManageFacilities() && $reservation->canBeEmergencyCancelled())
    <div class="collapse mb-3" id="emergency-box">
        <form method="POST" action="{{ route('fmo.reservations.emergency-cancel', $reservation) }}" class="p-3"
              style="background:var(--surface-2);border-left:4px solid var(--color-accent)">
            @csrf
            <div class="mb-2"><strong>Release this venue for an emergency</strong></div>
            <p class="tiny mb-3">
                The approval trail stays exactly as it is — nobody has to sign again. The date is freed immediately,
                and the requestor is asked to pick a replacement, in the system and by email.
            </p>

            <label class="form-label">Reason the requestor will see</label>
            <textarea name="emergency_reason" class="form-control mb-3" rows="3" required minlength="10"
                      placeholder="e.g. The Auditorium is needed for the Board of Trustees visit on the same date.">{{ old('emergency_reason') }}</textarea>

            <label class="form-label">What may the requestor change?</label>
            <div class="mb-3">
                <label class="d-block tiny mb-1"><input type="radio" name="emergency_options" value="both" checked> Either — a new date, or a different venue on the same date</label>
                <label class="d-block tiny mb-1"><input type="radio" name="emergency_options" value="date"> New date only — the venue stays the same</label>
                <label class="d-block tiny"><input type="radio" name="emergency_options" value="venue"> Different venue only — the date stays the same</label>
            </div>

            <button class="btn-reject" data-confirm="Release this venue and notify the requestor? Their approvals stay in place.">
                <i class="bi bi-exclamation-octagon"></i> Cancel booking and notify requestor
            </button>
        </form>
    </div>
    @endif
    ...
    <div class="surface p-3 mb-3" style="border-left:4px solid var(--color-accent)">
        ...
        @if($reservation->awaitingFmoRebooking())
            ...
            <div class="request-actions mb-2" style="justify-content:flex-start">
                <form method="POST" action="{{ route('fmo.reservations.rebooking.approve', $reservation) }}">@csrf
