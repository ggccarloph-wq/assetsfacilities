...
<form method="POST" action="{{ route('facilities.reservations.store') }}">@csrf
    <div class="row g-3">
      ...
      <div class="col-md-6"><label class="form-label">Start Date and Time</label><input type="datetime-local" name="start_at" class="form-control" min="{{ \App\Support\BookingWindow::minAttribute() }}" value="{{ old('start_at') }}" required></div>
